...
function detectDirtyTests(testFiles, dirtyFiles, filesMap) {
  const dirtySet = new Set(dirtyFiles);
  const out = [];
  for (const test of testFiles) {
    if (dirtySet.has(test)) {
      out.push(test);
      continue;
    }
...
function generateMap(fileEnding) {
  ...
  const dirtyTests = detectDirtyTests(testFiles, dirtyFiles, nextFiles);
